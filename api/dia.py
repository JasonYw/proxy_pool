...
def verify_ip(ip_):
    try:
        proxies = {"http": f"http://{PROXY_SERVICE_USERNAME}:{PROXY_SERVICE_PASSWORD}@{ip_}:{VPS_PORT}","https": f"http://{PROXY_SERVICE_USERNAME}:{PROXY_SERVICE_PASSWORD}@{ip_}:{VPS_PORT}",}
    ...
    if vps_conf['expired_time'] and vps_conf['env_is_ok']:
        if ttl_ < 30 and ttl_ != -2 and ttl_ != -1:
            if len(redis_.keys('vps_')) < 2:
                #保持池子不为空
                redis_.set_(f'vps_{vps_conf["vps_uuid"]}',redis_.get_(f'vps_{vps_conf["vps_uuid"]}'),120)
            else:
                ...
                while True:
                    ...
                    if ip_ and verify_ip(ip_):
                        redis_.set_(f'vps_{vps_conf["vps_uuid"]}',f'{ip_}:{VPS_PORT}',vps_conf['dia_frequency'])
                        break
                    else:
                        continue
...
def package_thread(threading_group,vps_conf,):
    ...
    if not threading_group.get(vps_conf['vps_uuid']):
        threading_group[vps_conf['vps_uuid']] = Thread(name=vps_conf['vps_uuid'],target=dia,args=(vps_conf,))
