...
def connect_redis():
    ...
    global REDISPOOL
    if not REDISPOOL:
        REDISPOOL = redis.Redis(connection_pool=redis.ConnectionPool(host=REDIS_HOST,port=int(REDIS_PORT),db=int(REDIS_DB),password=MYSQL_PASS,decode_responses=True))
    return REDISPOOL
...
class VpsConfigModel(BaseModel):
    ...
    def update_env(cls,vps_uuid):
        cls.cursor.execute(f'UPDATE {cls.__table} SET env_is_ok = true WHERE vps_uuid = {vps_uuid}')
        cls.__db.commit()
        cls.close()
